...
function filterByCity(city) {
  ...
  contacts.forEach(contact => {
    if(contact.address.city === city && !addedIds.includes(contact.id)){
...
function filterHandler() {
  ...
  filterOptions.addEventListener('change', (event) => {
    ...
    if(selectedCity === "0"){
      ...
      const filteredList = filterByCity(selectedCity);
      render(filteredList);
    }
  });
}
...
function loadCities(contacts) {
  // Initialize an empty array called cityList
  let cityList = [];
  contacts.forEach(contact=>{
      let cityName = contact.address.city;
  ...
  const defaultOption = document.createElement("option");
    ...
    defaultOption.innerText = "-- Select a city test --";
...
//   const contactId = document.querySelector(`[data-id="${id}"]`);
//   const contactList = document.querySelector("#contacts");
//    if(!contactList || !contactId){
//     return;
//   } 
//   if(contactId){
//  contactId.remove();
...
function deleteButtonHandler() {
  ...
  deleteBtns.forEach(button => {
    button.addEventListener("click", event => {
      const selectedElement = event.target.parentElement.getAttribute("data-id");
      deleteContact(selectedElement);
      loadCities(contacts);
      render(contacts);
  });
 });
